        ...
        for id in range(PRED_LEN):
            ...
            if id + 1 != PRED_LEN:
                if train_or_test == 0:
                    ...
                    speed = speed_control(pred_ped_speed[id + 1, :, :], 0, seq_start_end, pred_label[0, :, :])
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, obs_ped_speed, pred_ped_speed, pred_traj, train_or_test,
        ...
        if POOLING_TYPE:
            ...
            sspm = self.social_speed_pooling(final_encoder_h, seq_start_end, train_or_test, speed_to_add,
                                             obs_traj[-1, :, :], next_speed, pred_label[0, :, :], ped_features=pred_label)
        ...
        decoder_h = self.add_noise(noise_input, seq_start_end).view(NUM_LAYERS, batch, -1)
